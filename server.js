...
app.post('/roleplay', async (req, res) => {
    ...
    try {
        ...
        const response = await openai.chat.completions.create({ model: "gpt-4o-mini", messages: messages, temperature: 0.7, response_format: { type: "json_object" } });
        let cleanContent = response.choices[0].message.content.replace(/```json/gi, '').replace(/```/gi, '').trim();
        res.json(JSON.parse(cleanContent));
...
app.post('/save-report', async (req, res) => {
    ...
    // GIỮ NGUYÊN LINK CỦA BẠN TẠI ĐÂY
    ...
    try {
        ...
        console.log(`Đã gửi báo cáo: ${playerName} - ${type}`);
        res.json({ success: true });
    } catch (error) { 
        res.status(500).json({ success: false, message: "Lỗi lưu báo cáo." }); 
    }
});
